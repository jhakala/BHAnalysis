...
#Black Holes analysis extras - imported from patTemplate_cfg.
################ DO NOT MODIFY BETWEEN THESE LONG LINES ##############
...
if not options.useData :
	...
	if options.release == '42x' :
		...
		process.source.fileNames = [
			'file:/afs/cern.ch/user/a/aferapon/data1/BH/BlackHolesAnalysis_8TeV/SignalProduction/CMSSW_5_2_2/src/PrivateFastSim_withPU/BM1_MD1.5_M4.5_n2_cmssw522_fastsim_PU_1000evt.root'
	...
	if options.release == '42x':
		...
		process.source.fileNames = [
		    'rfio:/castor/cern.ch/cms/store/data/Run2012A/HT/AOD/PromptReco-v1/000/190/595/3026DAA7-2B82-E111-B21A-003048F024FA.root'
		    ]

#process.source.eventsToProcess = cms.untracked.VEventRange( ['1:86747'] )

#process.source.skipEvents = cms.untracked.uint32(40) 
...
# CA8 jets
process.selectedPatJetsCA8PF.cut = cms.string("pt > 50")

# CA8 Pruned jets
process.selectedPatJetsCA8PrunedPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")


# CA8 TopJets
process.selectedPatJetsCATopTagPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
...
if options.useExtraJetColls: 
	# CA12 Filtered jets
	process.selectedPatJetsCA12FilteredPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsCA12MassDropFilteredPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")

	# AK5 groomed jets
	process.selectedPatJetsAK5PrunedPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK5TrimmedPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK5FilteredPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")


	# AK7 groomed jets
	process.selectedPatJetsAK7PF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK7PrunedPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK7TrimmedPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK7FilteredPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")


	# AK8 groomed jets
	process.selectedPatJetsAK8PF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK8PrunedPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK8TrimmedPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	process.selectedPatJetsAK8FilteredPF.cut = cms.string("pt > 50 & abs(rapidity) < 2.6")
	


# electrons
process.selectedPatElectrons.cut = cms.string('pt > 50.0 & abs(eta) < 2.4')
process.patElectrons.embedTrack = cms.bool(True)
process.selectedPatElectronsPFlow.cut = cms.string('pt > 50.0 & abs(eta) < 2.4')
process.patElectronsPFlow.embedTrack = cms.bool(True)
process.selectedPatElectronsLoosePFlow.cut = cms.string('pt > 50.0 & abs(eta) < 2.4')
process.patElectronsLoosePFlow.embedTrack = cms.bool(True)
# muons
process.selectedPatMuons.cut = cms.string('pt > 50.0 & abs(eta) < 2.1')
process.patMuons.embedTrack = cms.bool(True)
process.selectedPatMuonsPFlow.cut = cms.string("pt > 50.0 & abs(eta) < 2.1")
process.patMuonsPFlow.embedTrack = cms.bool(True)
process.selectedPatMuonsLoosePFlow.cut = cms.string("pt > 50.0 & abs(eta) < 2.1")
process.patMuonsLoosePFlow.embedTrack = cms.bool(True)
# taus
process.selectedPatTausPFlow.cut = cms.string("pt > 50.0 & abs(eta) < 3")
process.selectedPatTaus.cut = cms.string("pt > 50.0 & abs(eta) < 3")
...
# BEGINNING of Black Hole analysis stuff
# User EDAnalyzer/EDFilter, overlaps removal (since photons are not particle flow)
##################################
...
#Tight photon ID
process.selectedPatPhotons.cut = cms.string("pt > 50 && abs(eta) < 2.4 && ecalRecHitSumEtConeDR04 < 4.2+0.006*pt && hcalTowerSumEtConeDR04 < 2.2+0.0025*pt && hadronicOverEm < 0.05 && trkSumPtHollowConeDR04 < 2+0.001*pt && !hasPixelSeed && ((sigmaIetaIeta<0.013&&isEB)||(sigmaIetaIeta<0.030&&!isEB))")
process.selectedPatMuonsPFlow.cut = cms.string("isGood('GlobalMuonPromptTight') && abs(eta) < 2.4")

process.selectedPatPhotons.checkOverlaps = cms.PSet(
     electrons = cms.PSet(
...
process.goodPatJetsPFlow.checkOverlaps = cms.PSet(
        muons = cms.PSet(
...
process.cleanPatPhotons.checkOverlaps.electrons.requireNoOverlaps = cms.bool(True)
process.goodPatJetsPFlow.checkOverlaps.muons.requireNoOverlaps = cms.bool(True)
process.goodPatJetsPFlow.checkOverlaps.electrons.requireNoOverlaps = cms.bool(True)
process.goodPatJetsPFlow.checkOverlaps.photons.requireNoOverlaps = cms.bool(True)

process.load('HLTrigger.HLTfilters.hltHighLevel_cfi')
process.hltHT = process.hltHighLevel.clone()
process.hltHT.HLTPaths = cms.vstring('HLT_*HT*')
process.hltHT.TriggerResultsTag = cms.InputTag("TriggerResults","","HLT")
process.hltHT.throw = cms.bool(False)

#process.TFileService = cms.Service("TFileService", fileName = cms.string("HT100U_PATTuples_Custom.root") )
process.TFileService = cms.Service("TFileService", fileName = cms.string("histo.root")) 
#Input collections
#Analysi source code - trees, used for debugging, scans etc
process.BHAnalyzer = cms.EDAnalyzer("BHAnalyzerPATTuplesTLBSM",                                                         
    ...
    jetTag      = cms.untracked.InputTag("goodPatJetsPFlow"),                                            
    ...
    #MC label will be used to turn OFF Mult>=2 cut to avoid MC normalization bias    
    MCLabel     = cms.untracked.bool(True),
    PtThreshold = cms.untracked.double(50),
    DEBUG	= cms.untracked.bool(False),
    DEBUGTRG	= cms.untracked.bool(False)                                                            
)                                                                                                       
#Analysis source code - histograms, used with final scripts
process.plots = cms.EDAnalyzer("SimpleAnalyzerTLBSM",
    jetSrc              = cms.InputTag("goodPatJetsPFlow"),
    electronSrc         = cms.InputTag("selectedPatElectronsPFlow"),
    photonSrc           = cms.InputTag("selectedPatPhotons"),
    muonSrc             = cms.InputTag("selectedPatMuonsPFlow"),
    metSrc              = cms.InputTag("patMETsPFlow"),
    metThreshold        = cms.double(50.0),
    DEBUG		= cms.bool(False)
)
# END of Black Hole analysis stuff
...
process.patseq = cms.Sequence(
    ...
    (process.BHAnalyzer+process.plots)
    )
